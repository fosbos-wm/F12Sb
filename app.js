...
async function renderLernwerkstatt(){
  const resources=[
         ["     ","Lernstrategien-Check","Kein Lerntyp-Test – dein Strategien-Profil in 13 Fragen.","lernstrategien"],
...
const LIST_ITEMS=[
  {id:"wiederholen",cat:"kognitiv",text:"Ich wiederhole Lernstoff mehrmals über mehrere Tage verteilt, statt nur einmal kurz vorher."},
  {id:"verknuepfen",cat:"kognitiv",text:"Ich verknüpfe neuen Lernstoff bewusst mit Dingen, die ich schon weiß."},
  {id:"organisieren",cat:"kognitiv",text:"Ich bringe Lerninhalte in eine übersichtliche Struktur (z. B. Mindmap, Gliederung, Tabelle)."},
  {id:"kritisch",cat:"kognitiv",text:"Ich hinterfrage, ob ich einen Inhalt wirklich verstanden habe, statt ihn nur auswendig zu wiederholen."},
  {id:"planen",cat:"metakognitiv",text:"Bevor ich anfange zu lernen, überlege ich kurz, was ich in dieser Einheit schaffen will."},
  {id:"ueberwachen",cat:"metakognitiv",text:"Während des Lernens merke ich selbst, wenn ich etwas nicht verstanden habe."},
  {id:"regulieren",cat:"metakognitiv",text:"Wenn eine Methode nicht funktioniert, wechsle ich bewusst zu einer anderen."},
  {id:"zeitmanagement",cat:"ressourcen",text:"Ich plane meine Lernzeit im Voraus, statt spontan zu entscheiden, wann ich lerne."},
  {id:"konzentration",cat:"ressourcen",text:"Es fällt mir leicht, mich beim Lernen über längere Zeit auf eine Sache zu konzentrieren."},
  {id:"anstrengung",cat:"ressourcen",text:"Auch bei schwierigen oder langweiligen Themen bleibe ich dran, statt aufzugeben."},
  {id:"umgebung",cat:"ressourcen",text:"Ich sorge bewusst für eine ablenkungsfreie Umgebung, wenn ich lerne (z. B. Handy weg)."},
  {id:"mitlernen",cat:"ressourcen",text:"Ich tausche mich mit anderen aus, um Lerninhalte besser zu verstehen (erklären, Fragen stellen)."},
  {id:"ressourcen",cat:"ressourcen",text:"Ich nutze gezielt zusätzliche Materialien (Bücher, Videos, Übungsaufgaben), wenn mir etwas unklar ist."}
];
const LIST_CATEGORIES={
  kognitiv:{label:"Kognitive Strategien",icon:"🧠",desc:"Wie du mit dem Lernstoff selbst umgehst.",methods:["Aktives Erinnern","Verteiltes Lernen","Mindmapping","Cornell-Methode","Feynman-Technik"]},
  metakognitiv:{label:"Metakognitive Strategien",icon:"🎯",desc:"Wie gut du dein eigenes Lernen planst und steuerst.",methods:["Lernreflexion"]},
  ressourcen:{label:"Ressourcenbezogene Strategien",icon:"⚙️",desc:"Wie du Zeit, Umgebung und Unterstützung nutzt.",methods:["Pomodoro-Technik","Gegenseitiges Erklären"]}
};
let listAnswers={};
...
function evaluateListTest(){
  ...
  el.innerHTML=`<div class="grid grid-3">${Object.entries(LIST_CATEGORIES).map(([cat,c])=>{
    const pct=byCat[cat];
    ...
    return `<div class="card" style="background:#f5f7f8"><span class="emoji">${c.icon}</span><strong>${esc(c.label)}</strong><p style="margin:4px 0 8px;font-size:12px;color:var(--muted)">${esc(c.desc)}</p><span class="pill ${level}">${pct}% · ${levelText}</span>${pct<75?`<p style="margin:10px 0 0;font-size:12px"><strong>Passende Methoden weiter oben:</strong> ${c.methods.map(esc).join(", ")}</p>`:""}</div>`;
  }).join("")}</div>`;
}
