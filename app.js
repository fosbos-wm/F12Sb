...
async function renderResilienz(){
 const emotionsMusikList=await getEmotionsMusik();
 const skills=[
 ...
 <style>
 .klang-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px}
 .klang-card{text-align:left;min-height:105px;padding:14px;cursor:pointer;transition:.25s cubic-bezier(.2,.8,.2,1);color:var(--ink);font:inherit;border-radius:16px;border:1px solid var(--line,#e2eaf0);box-shadow:0 2px 8px rgba(23,56,79,.05);display:flex;flex-direction:column;gap:6px}
 .klang-card:hover{transform:translateY(-3px);box-shadow:0 10px 20px rgba(23,56,79,.13)}
 .klang-card strong{font-size:12.5px;color:var(--blue-dark);font-weight:800}
 .klang-card small{font-size:10px;color:var(--muted);line-height:1.4}
 .klang-status{margin-top:auto;font-size:9.5px;padding:3px 9px;border-radius:999px;align-self:flex-start}
 .klang-playing{box-shadow:0 0 0 3px rgba(22,136,207,.3)!important;animation:klang-pulse 1.6s ease-in-out infinite}
 @keyframes klang-pulse{0%,100%{opacity:1}50%{opacity:.75}}
 @media(max-width:1000px){.klang-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
 .em-form{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:16px}
 .em-form select,.em-form input{border:1px solid #d6e2ea;border-radius:8px;padding:10px 12px;background:#fff;color:var(--ink);outline:none}
 .em-form input[type="url"]{flex:1;min-width:220px}
 .em-form input[type="text"]{flex:1;min-width:160px}
 .em-kategoria,.em-kategorie{margin-top:18px}
 .em-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:12px;margin-top:8px}
 .em-card{padding:10px;border-radius:14px}
 .em-card-foot{display:flex;align-items:center;gap:8px;margin-top:8px;position:relative}
 .em-code{width:44px;height:44px;border-radius:6px;flex:0 0 44px;border:1px solid var(--line,#e2eaf0)}
 .em-card-info{flex:1;min-width:0}
 .em-card-info strong{display:block;font-size:12px;color:var(--blue-dark)}
 .em-card-info p{margin:2px 0 0;font-size:11px;color:var(--muted);line-height:1.4}
 .em-remove{padding:2px 8px;font-size:11px;flex:0 0 auto}
 ...
 <div class="card"style="margin-top:16px">
 <div class="kicker">RESPRESSI · KLANGBIBLIOTHEK</div>
 <h2> Musik für dein Gefühl</h2>
 <p>Klänge wirken auf unsere Stimmung – wähle bewusst einen Klang für das Gefühl, das du gerade stärken möchtest. Kein Musikstück, sondern eigens erzeugte Klangfarben.</p>
 <div class="klang-grid">${KLANG_LIBRARY.map(k=>`<button class="card klang-card"style="background:${k.bg};border-left:4px solid ${k.border}"data-klang="${k.id}"onclick="playKlang('${k.id}')">
 <strong>${k.label}</strong><small>${k.desc}</small>
 <span class="pill klang-status">▶ Abspielen</span>
 </button>`).join("")}</div>
 <div class="form-actions"style="margin-top:10px"><button class="secondary"onclick="stopAllSounds()">Stopp</button></div>
 </div>

 <div class="card"style="margin-top:16px">
 <div class="kicker">RESPRESSI · MUSIK-SAMMLUNG</div>
 <h2> Was fühlst du dabei?</h2>
 <p>Teile einen Spotify-Link zu einem Song, der dir hilft, ein bestimmtes Gefühl herzustellen oder zu verarbeiten. Andere können direkt reinhören (30-Sek.-Vorschau) oder den Spotify-Code mit der Spotify-App scannen, um den ganzen Song zu hören.</p>
 <div class="em-form">
 <select id="emUrlEmotion"><option value="">Kategorie wählen …</option>${EMOTION_MUSIK_KATEGORIEN.map(k=>`<option value="${k.id}">${k.label}</option>`).join("")}</select>
 <input id="emUrlInput"type="url"placeholder="Spotify-Link einfügen (open.spotify.com/track/…)">
 <input id="emUrlNote"type="text"maxlength="120"placeholder="Kurzer Kommentar (optional)">
 <button class="primary"onclick="saveEmotionsMusik()">Teilen</button>
 </div>
 ${EMOTION_MUSIK_KATEGORIEN.map(k=>{
 const items=emotionsMusikList.filter(m=>m.emotion===k.id);
 return`<div class="em-kategorie">
 <div class="kicker"style="color:hsl(${k.hue},55%,42%)">${k.label} · ${items.length}</div>
 <div class="em-grid">${items.map(emotionsMusikCard).join("")||`<div class="empty">Noch nichts geteilt.</div>`}</div>
 </div>`;
 }).join("")}
 </div>
...
// Klangbibliothek: kurze, direkt im Code erzeugte Klangfarben (Web Audio API,
// keine Audiodatei, keine Urheberrechtsfrage) – jede auf ein bestimmtes
// Gefühl hin gestaltet. Kein Musikstück im klassischen Sinn, sondern gezielt
// komponierte Ton- und Klangfolgen.
const KLANG_LIBRARY=[
 {id:"freude",label:"Freude",desc:"Hell, beschwingt, aufwärtsstrebend",bg:"hsl(45,70%,93%)",border:"hsl(45,60%,55%)"},
 {id:"leichtigkeit",label:"Leichtigkeit",desc:"Luftig, schwebend, sanft",bg:"hsl(200,60%,94%)",border:"hsl(200,50%,60%)"},
 {id:"ruhe",label:"Ruhe",desc:"Tief, gleichmäßig, beruhigend",bg:"hsl(165,50%,93%)",border:"hsl(165,40%,50%)"},
 {id:"kraft",label:"Kraft & Zuversicht",desc:"Geerdet, stabil, warm",bg:"hsl(20,65%,93%)",border:"hsl(20,55%,55%)"},
 {id:"verbindung",label:"Verbindung",desc:"Zwei Stimmen im Einklang",bg:"hsl(330,55%,94%)",border:"hsl(330,45%,62%)"}
];
let klangAudioCtx=null;
let klangActiveNodes=[];
let klangStopTimer=null;
function klangGetCtx(){
 if(!klangAudioCtx)klangAudioCtx=new (window.AudioContext||window.webkitAudioContext)();
 return klangAudioCtx;
}
function klangTone(ctx,freq,startTime,duration,peakGain,type){
 ...
 gain.gain.linearRampToValueAtTime(peakGain,startTime+Math.min(0.25,duration*0.3));
 gain.gain.linearRampToValueAtTime(peakGain*0.6,startTime+duration*0.6);
 gain.gain.linearRampToValueAtTime(0.0001,startTime+duration);
 osc.start(startTime);
 osc.stop(startTime+duration+0.1);
 klangActiveNodes.push(osc);
}
function stopAllSounds(){
 klangActiveNodes.forEach(n=>{try{n.stop()}catch(e){}});
 klangActiveNodes=[];
 if(klangStopTimer){clearTimeout(klangStopTimer);klangStopTimer=null}
 document.querySelectorAll(".klang-card").forEach(el=>{
 el.classList.remove("klang-playing");
 const st=el.querySelector(".klang-status");if(st)st.textContent="▶ Abspielen";
 });
}
function playKlang(id){
 stopAllSounds();
 const ctx=klangGetCtx();
 ...
 notes.forEach((f,i)=>klangTone(ctx,f,now+rep*1.5+i*0.16,0.45,0.10,"triangle"));
 }
 }else if(id==="leichtigkeit"){
 const notes=[1046.5,1318.5,1568,2093];
 totalDur=5*2.2+2.6;
 for(let rep=0;rep<5;rep++){
 klangTone(ctx,notes[rep%notes.length],now+rep*2.2,2.6,0.07,"sine");
 }
 }else if(id==="ruhe"){
 totalDur=14;
 [130.81,164.81,196].forEach(f=>klangTone(ctx,f,now,14,0.055,"sine"));
 }else if(id==="kraft"){
 totalDur=10.5;
 [65.41,98,130.81].forEach(f=>klangTone(ctx,f,now,10,0.09,"sine"));
 for(let rep=0;rep<5;rep++)klangTone(ctx,196,now+rep*2,1.5,0.08,"triangle");
 }else if(id==="verbindung"){
 totalDur=5*2.2+2.4;
 for(let rep=0;rep<5;rep++){
 klangTone(ctx,392,now+rep*2.2,2.4,0.07,"sine");
 klangTone(ctx,493.88,now+rep*2.2+0.12,2.4,0.06,"sine");
 }
 }
 const btn=document.querySelector(`[data-klang="${id}"]`);
 if(btn){
 btn.classList.add("klang-playing");
 const st=btn.querySelector(".klang-status");if(st)st.textContent="… läuft";
 }
 klangStopTimer=setTimeout(stopAllSounds,totalDur*1000+300);
}
window.playKlang=playKlang;window.stopAllSounds=stopAllSounds;

// Musik-Sammlung "Was fühlst du dabei?": Schüler:innen teilen Spotify-Links
// pro Gefühlskategorie. Es wird nie eine Audiodatei gespeichert oder
// gehostet – nur der Link. Wiedergabe läuft über Spotifys eigenen,
// offiziellen Embed-Player (30-Sek.-Vorschau ohne Spotify-Login) plus einen
// echten, scanbaren Spotify-Code (über Spotifys öffentlichen Code-Dienst),
// mit dem man in der Spotify-App den vollständigen Song hört.
const EMOTION_MUSIK_KATEGORIEN=[
 {id:"freude",label:"Freude",hue:45},
 {id:"wut",label:"Wut abreagieren",hue:0},
 {id:"melancholie",label:"Melancholie",hue:220},
 {id:"trauer",label:"Trauer verarbeiten",hue:255},
 {id:"ruhe",label:"Ruhe & Entspannung",hue:165},
 {id:"kraft",label:"Kraft & Motivation",hue:25}
];
function parseSpotifyUrl(url){
 const m=String(url||"").match(/open\.spotify\.com\/(track|album|playlist|artist)\/([a-zA-Z0-9]+)/);
 if(!m)return null;
 return {type:m[1],id:m[2],uri:`spotify:${m[1]}:${m[2]}`};
}
async function getEmotionsMusik(){
 if(!db)return [];
 try{
 const snap=await getDocs(collection(db,"emotionsMusik"));
 return snap.docs.map(d=>({id:d.id,...d.data()}));
 }catch(e){console.error("Musik-Sammlung laden:",e);return []}
}
async function saveEmotionsMusik(){
 if(!isApproved()){toast("Nur freigeschaltete Nutzer können teilen.");return}
 const emotion=$("emUrlEmotion")?.value;
 const url=$("emUrlInput")?.value.trim();
 const note=$("emUrlNote")?.value.trim()||"";
 if(!emotion){toast("Bitte eine Kategorie wählen.");return}
 const parsed=parseSpotifyUrl(url);
 if(!parsed){toast("Das sieht nicht nach einem gültigen Spotify-Link aus (open.spotify.com/track/… o. Ä.).");return}
 try{
 await addDoc(collection(db,"emotionsMusik"),{
 uid:currentUser.uid,name:profile?.displayName||currentUser.email||"Campus-Mitglied",
 emotion,spotifyType:parsed.type,spotifyId:parsed.id,spotifyUri:parsed.uri,spotifyUrl:url,
 note,createdAt:serverTimestamp()
 });
 toast("Geteilt!");
 await render();
 }catch(e){console.error("Musik teilen:",e);toast("Konnte nicht gespeichert werden.")}
}
async function deleteEmotionsMusik(id){
 if(!confirm("Diesen Eintrag wirklich löschen?"))return;
 try{await deleteDoc(doc(db,"emotionsMusik",id));await render();toast("Gelöscht.")}
 catch(e){console.error("Musik löschen:",e);toast("Konnte nicht gelöscht werden.")}
}
function emotionsMusikCard(m){
 const mine=m.uid===currentUser.uid;
 const embedUrl=`https://open.spotify.com/embed/${m.spotifyType}/${m.spotifyId}`;
 const codeUrl=`https://scannables.scdn.co/uri/plain/svg/ffffff/black/300/${encodeURIComponent(m.spotifyUri)}`;
 return`<div class="card em-card">
 <iframe src="${embedUrl}"width="100%"height="152"frameborder="0"allow="encrypted-media"loading="lazy"style="border-radius:10px"></iframe>
 <div class="em-card-foot">
 <img src="${codeUrl}"alt="Spotify-Code zum Scannen"class="em-code"title="Mit der Spotify-App scannen, um den ganzen Song zu hören">
 <div class="em-card-info"><strong>${esc(m.name)}</strong>${m.note?`<p>${esc(m.note)}</p>`:""}</div>
 ${(mine||isTeacher())?`<button type="button"class="secondary em-remove"onclick="deleteEmotionsMusik('${m.id}')">✕</button>`:""}
 </div>
 </div>`;
}
window.saveEmotionsMusik=saveEmotionsMusik;window.deleteEmotionsMusik=deleteEmotionsMusik;
