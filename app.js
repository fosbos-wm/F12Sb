...
<strong>${t[1]}</strong><small>${t[2]}</small>${!t[4]?`<span class="badge" style="margin-top:8px">IN VORBEREITUNG</span>`:""}</a>`;
  return `${pageHead("ZUSAMMENARBEIT","Tools für Zusammenarbeit",
    "Kostenlose, direkt in die F12Sb integrierte Tools für Gruppenarbeit, Brainstorming und Unterricht – ganz ohne externe Anmeldung.",
...
async function downloadWordcloudPDF(wordcloudId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F12Sb · Wortwolke"+(cloud.description?" · "+cloud.description:"")
    );
...
async function downloadKanbanPDF(boardId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F12Sb · Kanban-Board"+(board.description?" · "+board.description:"")
    );
...
async function downloadTermPollPDF(pollId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F12Sb · Terminfindung · "+votes.length+" Stimme(n) insgesamt"+(poll.description?" · "+poll.description:"")
    );
...
async function downloadChecklistPDF(checklistId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F12Sb · Gemeinsame Checkliste · "+done+" von "+items.length+" erledigt"+(list.description?" · "+list.description:"")
    );
...
async function downloadAmpelPDF(roundId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F12Sb · Verständnis-Ampel · "+total+" Antwort(en)"+(round.description?" · "+round.description:"")
    );
...
async function downloadPollPDF(pollId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F12Sb · Live-Umfrage · "+total+" Stimme(n)"+(poll.description?" · "+poll.description:"")
    );
...
async function downloadDeckPDF(deckId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F12Sb · Karteikarten · "+cards.length+" Karte(n)"+(deck.description?" · "+deck.description:"")
    );
...
async function renderFachaufsatzUebersicht(){
  ...
  return `${pageHead("SELBSTSTÄNDIG LERNEN","Fachaufsatz-Training",
    "Fachaufsatz in Pädagogik/Psychologie üben – Baustein für Baustein, gegliedert nach den Lernbereichen 12.1–12.4.",
    `<button class="secondary" onclick="go('lernwerkstatt')">← Lernwerkstatt</button>
...
async function downloadEssayPDF(caseId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F12Sb · Fachaufsatz-Training"+(c.theoryArea?" · "+c.theoryArea:"")
    );
...
async function renderPinnwandUebersicht(){
  ...
  return `${pageHead("ZUSAMMENARBEIT","Pinnwand",
    "Digitale Pinnwände für Ideen, Brainstorming und Gruppenarbeit – im Raster, für die ganze F12Sb sichtbar.",
...
async function downloadBoardPDF(boardId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F12Sb · Pinnwand"+(board.description?" · "+board.description:"")
    );
...
function buildICS(events,calName){
  const lines=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//F12Sb//Kalender//DE","CALSCALE:GREGORIAN",
    `X-WR-CALNAME:${escapeICS(calName||"F12Sb Kalender")}`];
...
const META_PHASES={
  planung:{label:"Planung",icon:"🧭"},
  ueberwachung:{label:"Überwachung",icon:"👁️"},
  bewertung:{label:"Bewertung & Regulation",icon:"🔄"}
};
let metaAnswers={};
function renderMetaItems(){
  const el=$("metaItems");if(!el)return;
  el.innerHTML=META_ITEMS.map(it=>`<div style="padding:10px 0;border-bottom:1px solid var(--line,#eee)">
    <p style="margin:0 0 8px;font-size:14px">${esc(it.text)}</p>
    <div style="display:flex;flex-wrap:wrap;gap:6px">${LIST_SCALE.map(([val,label])=>`<button type="button" class="pill" style="cursor:pointer;padding:6px 12px;${metaAnswers[it.id]===val?"background:var(--blue,#1688cf);color:#fff":""}" onclick="setMetaAnswer('${it.id}','${val}')">${esc(label)}</button>`).join("")}</div>
  </div>`).join("");
}
function setMetaAnswer(id,val){metaAnswers[id]=val;renderMetaItems()}
function evaluateMetaTest(){
  const missing=META_ITEMS.filter(it=>metaAnswers[it.id]===undefined);
  const el=$("metaResult");if(!el)return;
  if(missing.length){toast(`Bitte noch ${missing.length} Aussage${missing.length===1?"":"n"} beantworten.`);return}
  const byPhase={};
  Object.keys(META_PHASES).forEach(ph=>{
    const items=META_ITEMS.filter(it=>it.phase===ph);
    const sum=items.reduce((s,it)=>s+Number(metaAnswers[it.id]),0);
    byPhase[ph]=Math.round((sum/(items.length*3))*100);
  });
  const barColor=pct=>pct>=75?"#10a94a":pct>=40?"#e8890c":"#d92c34";
  el.innerHTML=`<div class="card" style="background:#f5f7f8">${Object.entries(META_PHASES).map(([ph,p])=>{
    const pct=byPhase[ph];
    return `<div style="margin:12px 0"><div style="display:flex;justify-content:space-between;gap:8px;margin-bottom:5px"><strong>${p.icon} ${esc(p.label)}</strong><span>${pct}%</span></div><div style="height:14px;background:#e4e9ed;border-radius:7px;overflow:hidden"><div style="height:100%;width:${pct}%;background:${barColor(pct)};border-radius:7px"></div></div></div>`;
  }).join("")}</div>`;
}
window.setMetaAnswer=setMetaAnswer;window.evaluateMetaTest=evaluateMetaTest;

async function renderMetakognition(){
  return `${pageHead("SELBSTSTÄNDIG LERNEN","Metakognitive Lernstrategien","Denken über das eigene Denken – informieren und selbst einschätzen.")}
  <section class="card">
    <div class="kicker">🧠 WAS IST METAKOGNITION?</div>
    <h2 style="margin-top:4px">Über das eigene Lernen nachdenken</h2>
    <p style="color:var(--muted)">Metakognition bedeutet, das eigene Denken und Lernen bewusst wahrzunehmen und zu steuern – statt einfach drauflos zu lernen. Studien zeigen: Wie gut jemand sein eigenes Lernen plant, überwacht und bewertet, hängt oft enger mit dem Lernerfolg zusammen als reines Vorwissen oder Begabung. Die gute Nachricht: Metakognitive Fähigkeiten lassen sich trainieren.</p>
  </section>

  <div class="grid grid-3" style="margin-top:16px">
    <div class="card" style="background:#f5f7f8">
      <span class="emoji">🧭</span><strong>Planung</strong>
      <p style="margin:6px 0 0;font-size:13px;color:var(--muted)">Vor dem Lernen: Ziel klären, Vorwissen aktivieren, passende Strategie wählen, Zeit realistisch einschätzen.</p>
    </div>
    <div class="card" style="background:#f5f7f8">
      <span class="emoji">👁️</span><strong>Überwachung</strong>
      <p style="margin:6px 0 0;font-size:13px;color:var(--muted)">Während des Lernens: Verständnis aktiv prüfen, Fortschritt beobachten, Aufmerksamkeit kontrollieren.</p>
    </div>
    <div class="card" style="background:#f5f7f8">
      <span class="emoji">🔄</span><strong>Bewertung & Regulation</strong>
      <p style="margin:6px 0 0;font-size:13px;color:var(--muted)">Nach dem Lernen: Ergebnis einschätzen, Strategie anpassen, aus Fehlern konkrete Schlüsse ziehen.</p>
    </div>
  </div>

  <section class="card" style="margin-top:16px">
    <div class="kicker">🔬 WAS NACHWEISLICH GUT WIRKT</div>
    <h2 style="margin-top:4px">Konkrete Techniken</h2>
    <ul style="margin:8px 0 0;padding-left:18px;color:var(--muted);font-size:14px">
      <li style="margin-bottom:8px"><strong>Kalibrierung:</strong> die eigene Einschätzung „das hab ich verstanden" regelmäßig mit einer echten Übungsaufgabe abgleichen. Schwächere Lernende überschätzen ihr Verständnis erfahrungsgemäß häufiger – Kalibrierung macht das sichtbar.</li>
      <li style="margin-bottom:8px"><strong>Selbstbefragung:</strong> aktiv Fragen an sich selbst stellen statt passiv zu lesen – „Was ist der Kernpunkt? Könnte ich das erklären? Wo hab ich noch Lücken?"</li>
      <li style="margin-bottom:8px"><strong>Lerntagebuch:</strong> nach dem Lernen kurz notieren, was funktioniert hat und was nicht – macht Muster über die Zeit sichtbar (siehe auch: Lernreflexion bei den Lernmethoden).</li>
      <li style="margin-bottom:0"><strong>Checklisten vorher/nachher:</strong> vor dem Lernen kurz Ziel und Vorgehen notieren, danach abgleichen, ob es eingetroffen ist.</li>
    </ul>
  </section>

  <section class="card" style="margin-top:16px">
    <div class="kicker">🎯 SELBSTEINSCHÄTZUNG</div>
    <h2 style="margin-top:4px">Wie gut nutzt du die drei Phasen aktuell?</h2>
    <p style="color:var(--muted)">9 kurze Aussagen, 3 pro Phase. Kreuze an, wie oft das auf dich zutrifft.</p>
    <div id="metaItems" style="margin-top:14px"></div>
    <div class="form-actions" style="margin-top:14px">
      <button class="primary" onclick="evaluateMetaTest()">Auswertung anzeigen</button>
    </div>
...
const LERNSTAND_DEFAULTS = [
  {
    ...
    "nr": 1,
    "learningArea": "lb1",
    "areaTitle": "Entwicklung als lebensumfassenden, multifaktoriell beeinflussten Prozess begreifen und pädagogisch gestalten",
    "title": "Merkmale und Bedingungen von Entwicklung (Anlage, Umwelt, Selbststeuerung)",
    "description": "3 K-Prim-Aufgaben (je 5 Aussagen zum Ankreuzen) und 2 offene Fragen zu den Fallbeispielen „Lukas“, „Petra“ und „Judith“.",
    "tasks": [
      {
        "id": "fachwissen",
        "type": "kprim",
        "label": "K-Prim 1 – Merkmale des Entwicklungsbegriffs",
        "points": 3,
        "intro": "Beurteilen Sie jede der folgenden Aussagen zum Begriff „Entwicklung“. Setzen Sie ein Häkchen NUR bei den Aussagen, die Sie für RICHTIG halten.",
        "statements": [
          {
            "text": "Entwicklung bezeichnet eine gerichtete, zeitlich geordnete Reihe von Veränderungen des Erlebens und Verhaltens.",
            "correct": true,
            "explain": "Kernaussage der Definition (Hobmair)."
          },
          {
            "text": "Die Reihenfolge der Entwicklungsveränderungen ist prinzipiell umkehrbar.",
            "correct": false,
            "explain": "Die Reihenfolge ist gerade NICHT umkehrbar, z. B. krabbeln Kinder, bevor sie gehen können."
          },
          {
            "text": "Entwicklungsveränderungen lassen sich bestimmten Altersspannen zuordnen (Lebensalterbezogenheit).",
            "correct": true,
            "explain": "Das nennt man Lebensalterbezogenheit."
          },
          {
            "text": "Entwicklung bezieht sich ausschließlich auf die Kindheit und endet mit dem Erreichen des Erwachsenenalters.",
            "correct": false,
            "explain": "Entwicklung ist ein lebensumfassender Prozess (vgl. Lernbereichstitel 12.1) und endet nicht mit dem Erwachsenenalter."
          },
          {
            "text": "Die einzelnen Veränderungen im Entwicklungsverlauf treten unabhängig voneinander auf.",
            "correct": false,
            "explain": "Die Veränderungen bilden untereinander einen Zusammenhang und bauen aufeinander auf (z. B. Strampeln, Kriechen, Gehen)."
          }
        ],
      ...
      },
      {
        "id": "erkennen",
        "type": "kprim",
        "label": "K-Prim 2 – Die drei Entwicklungsbedingungen",
        "points": 3,
        "intro": "Beurteilen Sie jede der folgenden Aussagen zu Anlage, Umwelt und Selbststeuerung. Setzen Sie ein Häkchen NUR bei den Aussagen, die Sie für RICHTIG halten.",
        "statements": [
          {
            "text": "Mit „Anlage“ wird die genetische Ausstattung eines Menschen bezeichnet, die bei der Befruchtung festgelegt wird.",
            "correct": true,
            "explain": ""
          },
          {
            "text": "Die vier Bereiche der Umwelt sind die natürliche, kulturelle, ökonomische und soziale Umwelt.",
            "correct": true,
            "explain": ""
          },
          {
            "text": "Selbststeuerung bedeutet, dass der Mensch ausschließlich passiv auf Umwelteinflüsse reagiert.",
            "correct": false,
            "explain": "Der Mensch ist als aktives Wesen zu verstehen, das „von sich aus“ Entwicklungsprozesse herbeiführt."
          },
          {
            "text": "Anlage, Umwelt und Selbststeuerung wirken unabhängig voneinander und beeinflussen sich nicht gegenseitig.",
            "correct": false,
            "explain": "Die drei Bedingungen stehen in Wechselbeziehung zueinander und beeinflussen sich gegenseitig."
          },
          {
            "text": "Genetische Faktoren sind u. a. für sensible Phasen verantwortlich, in denen bestimmte Verhaltensweisen und Persönlichkeitsmerkmale optimal erworben werden können.",
            "correct": true,
            "explain": ""
          }
        ],
      ...
      },
      {
        "id": "analysieren",
        "type": "kprim",
        "label": "K-Prim 3 – Zusammenwirken der Bedingungen am Fall „Judith“",
        "points": 3,
        "intro": "Beurteilen Sie jede der folgenden Aussagen zum Fallbeispiel „Judith – ein Leben für den Schachsport“. Setzen Sie ein Häkchen NUR bei den Aussagen, die Sie für RICHTIG halten.",
        "statements": [
          {
            "text": "Judiths mathematisch-logische Begabung lässt sich als Hinweis auf einen Anlagefaktor deuten, da ihr Vater Mathematikprofessor ist.",
            "correct": true,
            "explain": "So die Argumentation der Lösungsskizze."
          },
          {
            "text": "Die Anlage entfaltet sich automatisch in vollem Umfang, unabhängig von Umwelteinflüssen.",
            "correct": false,
            "explain": "Das angelegte Potenzial muss erst durch entsprechende Umwelteinflüsse entfaltet werden."
          },
          {
            "text": "Ohne Judiths eigene Motivation (Selbststeuerung) hätte sich ihre Begabung vermutlich nicht in diesem Ausmaß entfalten können.",
            "correct": true,
            "explain": ""
          },
          {
            "text": "Die gesellschaftliche Anerkennung, die Judith durch das Schachspiel erhielt, zeigt, dass die Selbststeuerung unabhängig von der Umwelt wirkt.",
            "correct": false,
            "explain": "Das Beispiel zeigt im Gegenteil die Abhängigkeit/Wechselwirkung zwischen Selbststeuerung und Umwelt (Anerkennung von außen stützt die Motivation)."
          },
          {
            "text": "Anlage, Umwelt und Selbststeuerung stehen bei Judiths Entwicklung in wechselseitiger Abhängigkeit zueinander.",
            "correct": true,
            "explain": "Kernaussage zum „Zusammenspiel der Entwicklungsbedingungen“."
          }
        ],
  ...
  },
  {
    ...
    "learningArea": "lb1",
    "areaTitle": "Entwicklung als lebensumfassenden, multifaktoriell beeinflussten Prozess begreifen und pädagogisch gestalten",
    "title": "Psychoanalytische Entwicklungstheorie nach Freud",
  ...
  },
  {
    ...
    "learningArea": "lb1",
    "areaTitle": "Entwicklung als lebensumfassenden, multifaktoriell beeinflussten Prozess begreifen und pädagogisch gestalten",
    "title": "Sozialemotionale Entwicklung und Bindungstheorie (Bindungstypologie nach Ahnert)",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb1",
    "areaTitle": "Entwicklung als lebensumfassenden, multifaktoriell beeinflussten Prozess begreifen und pädagogisch gestalten",
    "title": "Entwicklungsaufgaben über die Lebensspanne nach Baltes",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb1",
    "areaTitle": "Entwicklung als lebensumfassenden, multifaktoriell beeinflussten Prozess begreifen und pädagogisch gestalten",
    "title": "Vulnerabilität und Resilienz",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "nr": 6,
    "learningArea": "lb2",
    "areaTitle": "Persönlichkeit und Identität beschreiben, erklären und reflektieren",
    "title": "Persönlichkeitsbegriff und die fünf Dimensionen („Big Five“)",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb2",
    "areaTitle": "Persönlichkeit und Identität beschreiben, erklären und reflektieren",
    "title": "Personenzentrierte Theorie nach Rogers",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb2",
    "areaTitle": "Persönlichkeit und Identität beschreiben, erklären und reflektieren",
    "title": "Sozialkognitive Theorie nach Bandura",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb2",
    "areaTitle": "Persönlichkeit und Identität beschreiben, erklären und reflektieren",
    "title": "Identitätsmodell nach Marcia",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "nr": 10,
    "learningArea": "lb3",
    "areaTitle": "Aufgaben und Arbeitsfelder Sozialer Arbeit professionell einordnen",
    "title": "Aufgabenbereiche Sozialer Arbeit (Sozialhilfe, Gesundheits-/Altenhilfe, Kinder-/Jugendhilfe)",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb3",
    "areaTitle": "Aufgaben und Arbeitsfelder Sozialer Arbeit professionell einordnen",
    "title": "Verhaltensorientiertes Konzept der Einzelhilfe",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb3",
    "areaTitle": "Aufgaben und Arbeitsfelder Sozialer Arbeit professionell einordnen",
    "title": "Ökologisches Konzept – Life Model nach Germain/Gitterman",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb3",
    "areaTitle": "Aufgaben und Arbeitsfelder Sozialer Arbeit professionell einordnen",
    "title": "Lebensweltorientierte Soziale Arbeit nach Thiersch",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "nr": 14,
    "learningArea": "lb4",
    "areaTitle": "In sozialen Beziehungen empathisch und zielführend kommunizieren und interagieren",
    "title": "Grundlagen der Kommunikation – Organon-Modell nach Bühler",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb4",
    "areaTitle": "In sozialen Beziehungen empathisch und zielführend kommunizieren und interagieren",
    "title": "Kommunikationstheorie nach Watzlawick (5 Axiome)",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb4",
    "areaTitle": "In sozialen Beziehungen empathisch und zielführend kommunizieren und interagieren",
    "title": "Entstehung von Kommunikationsstörungen",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb4",
    "areaTitle": "In sozialen Beziehungen empathisch und zielführend kommunizieren und interagieren",
    "title": "Kommunikationstechniken für gelungene Kommunikation",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
  ...
  },
  {
    ...
    "learningArea": "lb4",
    "areaTitle": "In sozialen Beziehungen empathisch und zielführend kommunizieren und interagieren",
    "title": "Kommunikation und Gefährdungen in digitalen Medien",
    "description": "Kompetenzüberprüfung mit fünf einheitlichen Kompetenzdimensionen.",
    "tasks": [
      {
        "id": "fachwissen",
...
async function renderLernstand(){
  ...
  return `${pageHead(
    ...
    "Lernstandsmessung",
    "19 Kompetenzüberprüfungen – mit einem einheitlichen Kompetenzprofil, damit deine Entwicklung sichtbar wird.",
...
async function downloadLernstandResultPDF(id){
  try{
    ...
    openToolPrintWindow(
      ...
      "F12Sb · Lernstandsmessung "+t.nr+"/19 · "+(LERNSTAND_AREAS[t.learningArea]?.title||"")
    );
