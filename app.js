...
// Die 34 Gemeinden des Landkreises Weilheim-Schongau mit ungefähren,
// relativ zueinander passenden Positionen (kein amtliches Kartenmaterial,
// sondern eine stilisierte, grob maßstabsgetreue Darstellung).
const LANDKREIS_ORTE=[
 ["Weilheim i.OB.",480,350],["Penzberg",680,380],["Schongau",180,330],
 ["Peißenberg",380,340],["Peiting",200,390],["Bernried a. Starnberger See",700,200],
 ["Hohenpeißenberg",390,400],["Pähl",600,230],["Polling",460,330],
 ["Raisting",560,250],["Wessobrunn",320,310],["Wielenbach",500,310],
 ["Altenstadt",150,420],["Hohenfurch",190,400],["Ingenried",140,460],
 ["Schwabbruck",130,380],["Schwabsoien",120,420],["Bernbeuren",160,490],
 ["Burggen",110,460],["Habach",520,460],["Antdorf",560,480],
 ["Obersöchering",500,430],["Sindelsdorf",580,440],["Huglfing",400,310],
 ["Eberfing",420,280],["Eglfing",380,270],["Oberhausen",350,350],
 ["Rottenbuch",250,440],["Böbing",270,400],["Seeshaupt",680,470],
 ["Iffeldorf",620,500],["Steingaden",180,550],["Prem",140,560],["Wildsteig",220,580]
];
...
function heimatkarteSVG(entries){
 ...
 return`<svg viewBox="0 0 800 620"style="width:100%;height:auto;max-height:480px">
 <path d="M90,330 Q70,230 220,180 Q330,110 460,140 Q620,110 730,190 Q790,260 740,360 Q760,470 640,540 Q520,610 380,590 Q230,620 150,540 Q60,470 90,330 Z"fill="var(--soft-green)"stroke="var(--green)"stroke-width="2"opacity=".5"/>
 ${dots}
 </svg>`;
}
...
async function renderStart(){
 let tasks=[],projects=[],news=[],nextCalendar=null,birthdayInfo=null,heimatEntries=[];
 try{[tasks,projects,news,nextCalendar,birthdayInfo,heimatEntries]=await Promise.all([getCollection("tasks","deadline",false),getCollection("projects"),getCollection("news"),getUpcomingCampusCalendarEvent(),getUpcomingBirthdayInfo(),getHeimatEintraege()])}catch(e){}
...
class="list-item"><div><strong>${esc(p.title||p.text)}</strong>${p.title?`<small>${esc(p.text)} · ${fmtDate(p.createdAt)}</small>`:`<small>${fmtDate(p.createdAt)}</small>`}</div><div style="display:flex;align-items:center;gap:8px"><span class="pill">Info</span>${isAdmin()?`<button class="secondary"onclick="deleteNews('${p.id}')">Löschen</button>`:""}</div>
 ...
 <p style="color:var(--muted)">Trag deinen Heimatort im Landkreis Weilheim-Schongau ein – so seht ihr auf einen Blick, wer aus eurer Gegend kommt. Vielleicht ergeben sich daraus ja Fahrgemeinschaften oder gemeinsame Projekte übers Jahr.</p>
